...
// Hidden-column selections per slice, keyed by slice_id. Survives chart
// unmount/remount within a page session (e.g. dashboard tab switches) but
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  // Reinitialise from cache when slice_id changes mid-lifecycle (e.g. an
  // embedded dashboard that swaps chart context without unmounting).
  useEffect(() => {
    setHiddenColumns(new Set(columnSelectionCache.get(slice_id) ?? []));
  }, [slice_id]);
  ...
  // Pickable columns are pre-filtered by chart editor
  const selectableColumnsMeta = useMemo(
    () => filteredColumnsMeta.filter(col => col.config?.visible !== false),
    [filteredColumnsMeta],
  );

  // Apply visibility toggle on top of pickable columns
  ...
  const renderColumnSelector = useCallback(
    ...
    [columnSelectorOptions, hiddenColumns, handleHiddenColumnsChange],
  );
